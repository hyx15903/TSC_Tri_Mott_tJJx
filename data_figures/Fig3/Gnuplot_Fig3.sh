...
plot "Fig3a.dat" index 0 using 4:5 w lp title "$M=6000$" ls 1,\
"Fig3a.dat" index 1 using 4:5 w lp title "$M=8000$" ls 2,\
"Fig3a.dat" index 2 using 4:5 w lp title "$M=10000$" ls 3,\
"Fig3a.dat" index 3 using 4:5 w lp title "$M=12000$" ls 4,\
"Fig3a.dat" index 4 using 1:2 w lp title "$M\\rightarrow \\infty $" ls 5
...
c(x) = c0+c1*x
fit c(x) "Fig3b.dat" index 0 u (log($1)):(log(abs($2))) every 4::5::14 via c0,c1

d(x) = d0+d1*x
fit d(x) "Fig3b.dat" index 1 u (log($1)):(log(abs($2))) every 4::5::14 via d0,d1

set label sprintf("$K _{G}=%.2f$", -c1) at 4,3e-2
set label sprintf("$K _{CDW}=%.2f$", -d1) at 1.05,2e-4

plot "Fig3b.dat" index 0 using 1:(abs($2)) w lp title "$\\left |G(r)\\right |$" ls 1,\
exp(c0)*x**c1 notitle ls 2,\
"Fig3b.dat" index 1 using 1:(abs($2)) w lp title "$\\left |D(r)\\right |$" ls 3,\
...
unset xtics
unset ytics
plot "Fig3b.dat" index 2 using 1:(abs($2)) every ::::13 w lp title "$\\left |G(r)\\right |$" ls 11,\
"Fig3b.dat" index 3 using 1:(abs($2)) every ::::13 w lp title "$\\left |D(r)\\right |$" ls 13
...
w(x) = w0+w1*x
fit w(x) "Fig3c.dat" index 0 using 3:(log(abs($4))) every 4::8::17 via w0,w1

c(x) = c0+c1*x
fit c(x) "Fig3c.dat" index 2 u 1:(log(abs($2))) every 4::5::18 via c0,c1

set label sprintf("$\\xi _{G}=%.2f$", -1./w1) at 13,1.2e-4 rotate by -30

set label sprintf("$\\xi _{G}=%.2f$", -1./c1) at 13,7e-3 rotate by -10.6

plot "Fig3c.dat" index 0 using 3:(abs($4)) w lp title "$L_{x}=36$" ls 1,\
"Fig3c.dat" index 1 using 3:(abs($4)) every ::::13 w lp title "$L_{x}=24$" ls 11,\
...
plot "Fig3c.dat" index 2 using 1:(abs($2)) w lp title "$L_{x}=36$" ls 3,\
"Fig3c.dat" index 3 using 1:(abs($2)) every ::::13 w lp title "$L_{x}=24$" ls 12
...
set border lw 4
b(x) = b0+b1*x
fit b(x) "Fig3d.dat" index 1 u 1:(log(abs($2))) every 2::1::11 via b0,b1

k(x) = k0+k1*x
fit k(x) "Fig3d.dat" index 3 u 1:(log(abs($2))) every 6::5::24 via k0,k1
...
plot "Fig3d.dat" index 0 using 1:(abs($2)) every ::::13 w lp title "$L_{x}=24$" ls 11,\
"Fig3d.dat" index 1 using 1:(abs($2)) w lp title "$L_{x}=36$" ls 1,\
...
plot "Fig3d.dat" index 2 using 1:(abs($2)) every ::::13 w lp title "$L_{x}=24$" ls 12,\
"Fig3d.dat" index 3 using 1:(abs($2)) w lp title "$L_{x}=36$" ls 3,\
