...
a(x) = m0+m1*x
fit a(x) "Fig4a.dat" index 0 u (log($4)):(log($5)) every 5::6::23 via m0,m1

b(x) = n0+n1*x
fit b(x) "Fig4a.dat" index 1 u (log($4)):(log($5)) every 6::8::23 via n0,n1

#set title "doping=1/12, 24*6, t=3, $J_{2}=0.1, J_{\\chi}=0.1$, aa-bond"
plot "Fig4a.dat" index 0 using 4:5 w lp title "$J_{2}=0.01$" ls 3,\
"Fig4a.dat" index 1 using 4:5 w lp title "$J_{2}=0$" ls 1
...
d(x) = d0+d1*x
fit d(x) "Fig4b.dat" index 0 u (log($1)):(log(abs($2))) every 6::3::16 via d0,d1

e(x) = e0+e1*x
fit e(x) "Fig4b.dat" index 1 u (log($1)):(log(abs($2))) every 6::3::23 via e0,e1

plot "Fig4b.dat" index 0 using 1:(abs($2)) w lp title "$J_{2}=0.01$" ls 3,\
"Fig4b.dat" index 1 using 1:(abs($2)) w lp title "$J_{2}=0$" ls 1
...
c(x) = c0+c1*x
fit c(x) "Fig4c.dat" index 0 u 3:(log(abs($4))) every 4::8::18 via c0,c1

d(x) = d0+d1*x
fit d(x) "Fig4c.dat" index 1 u 3:(log(abs($4))) every 7::7::18 via d0,d1

plot "Fig4c.dat" index 0 using 3:(abs($4)) w lp title "$J_{2}=0.01$" ls 3,\
"Fig4c.dat" index 1 using 3:(abs($4)) w lp title "$J_{2}=0$" ls 1
...
k(x) = e0+e1*x
fit k(x) "Fig4d.dat" index 0 u 3:(log(abs($4))) every 6::6::24 via e0,e1

l(x) = f0+f1*x
fit l(x) "Fig4d.dat" index 1 u 3:(log(abs($4))) every 4::3::24 via f0,f1
...
plot "Fig4d.dat" index 0 using 3:(abs($4)) w lp title "$J_{2}=0.01$" ls 3,\
"Fig4d.dat" index 1 using 3:(abs($4)) w lp title "$J_{2}=0$" ls 1
